#!/usr/bin/env python3
"""
Simple Global Cone Map Visualizer
Shows confirmed cone positions in the global map
"""
...
class GlobalConeVisualizer(Node):
    """Simple visualizer for global cone map"""
    
    def __init__(self):
        super().__init__('global_cone_visualizer')
        ...
        self.get_logger().info('Global Cone Visualizer initialized')
...
def create_global_plot():
    """Create and run global cone visualization"""
    
    # Initialize ROS2
    rclpy.init()
    visualizer = GlobalConeVisualizer()
    ...
    ax.set_title('Global Cone Map (Confirmed Cones)', fontsize=14, fontweight='bold')
    ...
    def update_plot(frame):
        ...
        for color, cones in data['global_cones'].items():
            if cones:
                ...
                # Plot cones (uniform size since these are confirmed)
                ax.scatter(x_coords, y_coords, c=color, s=80, 
                          alpha=0.9, marker='o', label=f'{color.title()} cones',
        ...
        # Adaptive zoom based on cone distribution
        if total_cones > 0:
            # Get all cone positions
            all_x = []
            all_y = []
            for color_cones in data['global_cones'].values():
            ...
            if all_x and all_y:
                ...
                # Default zoom around vehicle
                zoom = 20.0
                ax.set_xlim(vx - zoom, vx + zoom)
                ax.set_ylim(vy - zoom, vy + zoom)
        else:
            # Default zoom around vehicle when no cones
            zoom = 20.0
            ax.set_xlim(vx - zoom, vx + zoom)
            ax.set_ylim(vy - zoom, vy + zoom)
        ...
        # Add cone count
        ax.text(0.02, 0.98, f'Total Global Cones: {total_cones}', 
